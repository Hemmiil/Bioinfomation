...
class FlowCytometryProcessor:
    ...
    def _get_paths(self):
        ...
        with open('../CellSorter_rawdata/dir_ward_dict.json') as f:
            dir_ward_dict = json.load(f)

        pathes_dict = dict()
        for date, ward  in dir_ward_dict.items():
            pathes_dict_sub = dict()
            for station in [1,4,8]:
                pathes = glob.glob(f"{self.base_path}/{date}/user1_Experiment*s{station}*.csv")
                if len(pathes) > 0:
                    pathes_dict_sub[station] = pathes
                else:
                    pathes_dict_sub[station] = []

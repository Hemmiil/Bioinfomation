...
class FlowCytometryProcessor:
    ...
    def _get_paths(self):
        """指定されたステーションに基づいてファイルパスを取得"""
        pathes_dict = dict()
        for station in self.stations:
            pathes = glob.glob(f"{self.base_path}/*/user1_Experiment*_s{station}*.csv")
            pathes.sort()
            pathes_dict[station] = pathes
        return pathes_dict
    ...
    def process_all(self):
        ...
        for station, pathes in self.pathes_dict.items():
            for path in pathes:
                df_sub = self.process_file(path, station)
        ...
        df_ward["DATE"] = self.df["DATE"].apply(lambda x: x[:-3])
        self.df["ward"] = df_ward["DATE"].str.cat(df_ward["STATION_label"], sep="-s")

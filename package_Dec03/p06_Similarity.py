...
def heatmap_manual(df_result, tax_key, n_conpinents, dir_path="tmp", is_save=False):
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
...
def qqplot_manual(df_result, tax_key, n_conpinents, dir_path="tmp", is_save=False):
    r = df_result["r-statics"].fillna(0).replace(1, 1-1e-6)
